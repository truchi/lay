...
macro_rules! saturating {
    ($($(#[$meta:meta])* $Trait:ident $fn:ident)*) => {
        saturating!($($(#[$meta])* $Trait $fn [u8 i8 u16 i16 u32 i32 u128 i128 usize isize f32 f64])*);
    };
    ($($(#[$meta:meta])* $Trait:ident $fn:ident [$($T:ty)*])*) => { $(
        $(#[$meta])*
        pub trait $Trait<Rhs = Self> {
        ...
        $(impl<Rhs: Into<Self>> $Trait<Rhs> for $T {
            type Output = Self;
            fn $fn(self, rhs: Rhs) -> Self::Output { self.$fn(rhs.into()) }
        })*
    )* };
}
